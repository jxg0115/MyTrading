...
public class Program
{
	...
	private static async Task<BacktestResult> RunBacktest(BacktestRequest req)
	{
		...
		var volume = Math.Max(0.01m, req.Volume <= 0 ? 1 : req.Volume);   // lot 语义（fixed 模式由 BlockStrategy 换算）

		// 积木引擎：优先使用完整配置；旧客户端的字段自动组装成配置
		string configJson;
		if (req.ConfigJson.ValueKind == System.Text.Json.JsonValueKind.Object)
			configJson = req.ConfigJson.GetRawText();
		else
		{
			object entry;
			if (req.Strategy == "breakout")
				entry = new { type = "breakout", @params = new { length = Math.Max(2, req.Length) } };
			else
				entry = new { type = "sma_cross", @params = new { fast = Math.Max(2, req.Fast), slow = Math.Max(3, req.Slow) } };
			configJson = System.Text.Json.JsonSerializer.Serialize(new
			{
				entry,
				exit = new { stop = req.Stop ?? 0m, take = req.Take, trail = req.Trail, partial = req.PartialAt },
				position = new { mode = "fixed", lots = volume },
			});
		}

		var strategy = new BlockStrategy(configJson, ContractSize(symbol))
		{
