...
function setCandles(candles) {
  ...
  // 叠加两条 SMA 展示线（仅参考；回测信号由后端真实指标计算）
...
async function runBacktest() {
  ...
  if (!selStrategy) { alert("请先在策略下拉中选择一个策略（可到策略库新建）"); btn.disabled = false; document.getElementById("progress").classList.add("hidden"); return; }
  const body = {
...
function render(r) {
  const markers = r.trades
    ...
    .sort((a, b) => a.time - b.time);   // epoch数字排序（字符串排序对数字会抛异常）
...
function fmt(name, v) {
  ...
  if (abs.includes(name)) return n.toLocaleString("en-US", { maximumFractionDigits: 2 });
...
function fillForm(s) {
  // 统一兼容：记录字段是 paramsJson，卡片是 params
  const rec = {
    id: s.id ?? 0,
    name: s.name ?? "未命名策略",
    template: s.template,
    ...
    tf: s.tf,
    notes: s.notes ?? "",
    params: s.params || s.paramsJson || "{}",
  };
  selStrategy = rec;

  const p = JSON.parse(rec.params);
  document.getElementById("strategy").value = String(rec.id);
  document.getElementById("symbol").value = rec.symbol;
  document.getElementById("tf").value = rec.tf;
  const isSma = rec.template === "sma";
  ...
  banner.textContent = `📌 当前策略：${rec.name}（${rec.symbol} ${rec.tf}）` + (rec.notes ? ` · ${rec.notes}` : "");
...
async function refreshStrategySelect(preferId) {
  ...
  if (target) {
    ...
    if (s) applyStrategyRecord(s, true);
  } else {
    selStrategy = null;
    document.getElementById("smaParams").classList.add("hidden");
    document.getElementById("boParams").classList.add("hidden");
  }
}

function applyStrategyRecord(s, alsoFillForm) {
  if (alsoFillForm) fillForm(s);
}
...
async function refreshStrategies() {
  ...
  const statusZh = { draft: "\u{1F4DD}\u8349\u6848", tested: "\u2705\u5DF2\u56DE\u6D4B", paper: "\u{1F9EA}\u6A21\u62DF\u9A8C\u8BC1", retired: "\u{1F5D1}\u9000\u5F79" };
  ...
  }).join("") || '<tr><td colspan="7" class="muted">\u7B56\u7565\u5E93\u4E3A\u7A7A\uFF1A\u5728\u56DE\u6D4B\u5DE5\u4F5C\u53F0\u56DE\u6D4B\u540E\u70B9"\u5B58\u5165\u7B56\u7565\u5E93"\uFF0C\u6216\u70B9\u4E0A\u65B9"\u65B0\u5EFA\u7B56\u7565"</td></tr>';
}
...
function openStrModal(id) {
  ...
  if (id == null) {
    ...
    const f = collectForm();
    const label = ENTRY_DEFS[f.config.entry.type]?.label ?? f.config.entry.type;
    document.getElementById("saveName").value = `${f.symbol}${f.tf} ${label}`;
    document.getElementById("saveModal").classList.remove("hidden");
  });
  h3.appendChild(btn);
}
setTimeout(installSaveEntry, 0);
...
async function refreshLiveStrategySelect(preferId) {
  ...
  if (target) {
    ...
    selLiveStrategy = null;
    document.getElementById("lSmaParams").classList.add("hidden");
    document.getElementById("lBoParams").classList.add("hidden");
  }
}
...
function applyLiveStrategy(s) {
  ...
  const banner = document.getElementById("liveStrategyBanner");
  banner.textContent = `📌 ${s.name}（${s.symbol} ${s.tf}）` + (s.notes ? ` · ${s.notes}` : "");
}
...
document.getElementById("lStop").addEventListener("click", async () => {
  ...
  document.getElementById("liveErr").textContent = "已停止（若为刷新前启动的引擎，进程已一并终止）";
});
...
async function pollLive() {
  ...
  document.getElementById("liveQuote").textContent =
    s.bid ? ` 买 ${s.ask} / 卖 ${s.bid}` : "";
  document.getElementById("liveUpdated").textContent =
    `策略:${s.strategy} 更新于 ${s.last_update}`;
...
document.getElementById("optBtn").addEventListener("click", () => {
  ...
  const isSma = st === "sma";
  document.getElementById("optALabel").textContent = isSma ? "快线" : "突破天数";
  document.getElementById("optBLabel").textContent = isSma ? "慢线" : "止损%";
  document.getElementById("thA").textContent = isSma ? "快线" : "突破天数";
  document.getElementById("thB").textContent = isSma ? "慢线" : "止损%";
  document.getElementById("optRangeHint").textContent =
    `目标: ${document.getElementById("symbol").value.toUpperCase()} ${document.getElementById("tf").value} · ${document.getElementById("strategy").selectedOptions[0].text}`;
  optModal.classList.remove("hidden");
});

document.getElementById("optClose").addEventListener("click", () => optModal.classList.add("hidden"));
...
document.getElementById("optRun").addEventListener("click", async () => {
  ...
  const body = {
    ...
    volume: +document.getElementById("volume").value,
    stop: +document.getElementById("stop").value,
    aFrom: +document.getElementById("optAFrom").value,
    aTo: +document.getElementById("optATo").value,
    aStep: +document.getElementById("optAStep").value,
    bFrom: +document.getElementById("optBFrom").value,
    bTo: +document.getElementById("optBTo").value,
    bStep: +document.getElementById("optBStep").value,
  };
  ...
  document.getElementById("optBar").classList.remove("hidden");
  if (optTimer) clearInterval(optTimer);
  optTimer = setInterval(pollOpt, 1500);
  pollOpt();
});
...
async function pollOpt() {
  ...
  document.getElementById("optProgress").textContent = `${s.done}/${s.total} (${pct}%)`;
  document.getElementById("optBarFill").style.width = pct + "%";

  if (!s.running && s.done > 0) {
    clearInterval(optTimer);
    optTimer = null;
    document.getElementById("optBar").classList.add("hidden");
    renderOptResults(s.results);
  }
}
...
function renderOptResults(results) {
  const parsed = results.map(r => ({
    a: r.a, b: r.b,
    net: parseFloat(r.netProfit), sharpe: parseFloat(r.sharpe),
    dd: parseFloat(r.maxDd), pf: parseFloat(r.pf), trades: +r.trades,
  })).sort((x, y) => y.net - x.net);   // 按净利润降序

  const tb = document.querySelector("#optTable tbody");
  tb.innerHTML = parsed.map(r => `<tr class="optRow" data-a="${r.a}" data-b="${r.b}">
    <td>${r.a}</td><td>${r.b}</td>
    <td class="${r.net >= 0 ? "pos" : "neg"}">${(r.net ?? 0).toLocaleString("en-US", { maximumFractionDigits: 0 })}</td>
    <td>${(r.sharpe ?? 0).toFixed(3)}</td>
    <td class="neg">${(r.dd ?? 0).toFixed(2)}</td>
    <td>${(r.pf ?? 0).toFixed(2)}</td>
    <td>${r.trades}</td>
  </tr>`).join("");
  document.getElementById("optTable").classList.remove("hidden");
  document.getElementById("optHint2").classList.remove("hidden");

  // 点击行 -> 参数填回左侧
  tb.querySelectorAll(".optRow").forEach(row => {
    row.addEventListener("click", () => {
      const a = row.dataset.a, b = row.dataset.b;
      const st = document.getElementById("strategy").value;
      if (st === "sma") {
        document.getElementById("fast").value = a;
        document.getElementById("slow").value = b;
      } else {
        document.getElementById("length").value = a;
        document.getElementById("stop").value = b;
      }
...
async function autoLoadLatestConv() {
  const list = await (await fetch("/api/ai/conversations")).json();
  if (!list.length || aiConvId) return;   // 已有进行中的会话就不打断
  aiConvId = list[0].id;
  refreshConvList();
  // 触发 change 加载消息
  document.getElementById("aiConvList").value = aiConvId;
  document.getElementById("aiConvList").dispatchEvent(new Event("change"));
}
...
// 卡片 -> 直接回测：先自动存入策略库，再选中并回测
document.getElementById("aiCardBacktest").addEventListener("click", async () => {
  if (!aiCard) return;
  const pr = aiCard.params || {};
  const rec = {
    name: aiCard.name || "AI策略",
    template: aiCard.template === "breakout" ? "breakout" : "sma",
    symbol: (aiCard.symbol || curSymbol).toUpperCase(),
    tf: aiCard.tf || curTf,
    paramsJson: JSON.stringify({
      ...(aiCard.template === "sma"
        ? { fast: pr.fast ?? 10, slow: pr.slow ?? 30 }
        : { length: pr.length ?? 20 }),
      volume: pr.volume ?? 10000, stop: pr.stop ?? 2,
      take: pr.take ?? 0, trail: pr.trail ?? 0, partial: pr.partial ?? 0,
    }),
    status: "draft",
    tags: "AI生成",
  ...
  await refreshStrategySelect(saved.id);
  fillForm(saved);
  switchView("backtest");
  if (aiCard.template === "sma") {
    document.getElementById("fast").value = pr.fast ?? 10;
    document.getElementById("slow").value = pr.slow ?? 30;
  } else {
    document.getElementById("length").value = pr.length ?? 20;
  }
  document.getElementById("volume").value = pr.volume ?? 1;
  document.getElementById("stop").value = pr.stop ?? 2;
  document.getElementById("take").value = pr.take ?? 0;
  document.getElementById("trail").value = pr.trail ?? 0;
  document.getElementById("partial").value = pr.partial ?? 0;
  switchView("backtest");
});
...
document.getElementById("aiCardSave").addEventListener("click", async () => {
  ...
  const rec = {
    ...
    symbol: (aiCard.symbol || "EURUSD").toUpperCase(),
    tf: aiCard.tf || "D1",
...
function cardToText(card) {
  const p = card.params || {};
  let exit = `止损 ${p.stop ?? 0}%`;
  if (p.take > 0) exit += `；浮盈达 ${p.take}% 全部止盈落袋`;
  if (p.trail > 0) exit += `；启用【移动止损】：利润回撤 ${p.trail}% 就自动落袋（让利润奔跑）`;
  if (p.partial > 0) exit += `；浮盈达 ${p.partial}% 时先【平掉一半】落袋，剩余继续持有`;
  const lot = `每笔 ${p.volume ?? 10000} 单位。${exit}。`;
